extends Area2D

@onready var sprite_2d: Sprite2D = $Sprite2D

const NINA_DIA_1_1 = preload("uid://cxvbj32eopvqq")
const NINA_DIA_1_2 = preload("uid://bh2e685ff24te")
const NINA_DIA_2_1 = preload("uid://bvnycw2u3dmis")
const NINA_DIA_2_2 = preload("uid://bbb0t78wa03m6")
const NINA_DIA_3_1 = preload("uid://7twlr3bhp7vm")
const NINA_DIA_3_2 = preload("uid://cdagcsshwfgxe")
const NINA_DIA_4_1 = preload("uid://dalc8lxywa783")
const NINA_DIA_4_2 = preload("uid://bpum1qfwhqra8")
const NINA_DIA_5_1 = preload("uid://bu361rdon1fgc")
const NINA_DIA_5_2 = preload("uid://dxur1dxcr2m1w")
const NINA_DIA_6_1 = preload("uid://lkq7usd1651u")
const NINA_DIA_6_2 = preload("uid://bnxidyp4ffdtk")
const NINA_DIA_7_1 = preload("uid://6qpcxk5ymckb")
const NINA_DIA_7_2 = preload("uid://iju3mll11rha")




var player_close: bool = false

func _ready() -> void:
	DialogueManager.dialogue_started.connect(_on_dialogue_started)
	DialogueManager.dialogue_ended.connect(_on_dialogue_ended)
	
func _process(delta):
	if player_close and Input.is_action_just_pressed("ui_accept") and not GameManager.is_dialogue_active:
		if GameManager.dia == 1:
			if not Global.dia1_hablar_nina:
				DialogueManager.show_dialogue_balloon(NINA_DIA_1_1, "start")
			elif Global.dia1_hablar_nina:
				DialogueManager.show_dialogue_balloon(NINA_DIA_1_2, "start")
		elif GameManager.dia == 2:
			if not Global.dia2_hablar_nina:
				DialogueManager.show_dialogue_balloon(NINA_DIA_2_1, "start")
			elif Global.dia2_hablar_nina:
				DialogueManager.show_dialogue_balloon(NINA_DIA_2_2, "start")
		elif GameManager.dia == 3:
			if not Global.dia3_hablar_nina:
				DialogueManager.show_dialogue_balloon(NINA_DIA_3_1, "start")
			elif Global.dia3_hablar_nina:
				DialogueManager.show_dialogue_balloon(NINA_DIA_3_2, "start")
		elif GameManager.dia == 4:
			if not Global.dia4_hablar_nina:
				DialogueManager.show_dialogue_balloon(NINA_DIA_4_1, "start")
			elif Global.dia4_hablar_nina:
				DialogueManager.show_dialogue_balloon(NINA_DIA_4_2, "start")
		elif GameManager.dia == 5:
			if not Global.dia5_hablar_nina:
				DialogueManager.show_dialogue_balloon(NINA_DIA_5_1, "start")
			elif Global.dia5_hablar_nina:
				DialogueManager.show_dialogue_balloon(NINA_DIA_5_2, "start")
		elif GameManager.dia == 6:
			if not Global.dia6_hablar_nina:
				DialogueManager.show_dialogue_balloon(NINA_DIA_6_1, "start")
			elif Global.dia6_hablar_nina:
				DialogueManager.show_dialogue_balloon(NINA_DIA_6_2, "start")
		elif GameManager.dia == 7:
			if not Global.dia7_hablar_nina:
				DialogueManager.show_dialogue_balloon(NINA_DIA_7_1, "start")
			elif Global.dia7_hablar_nina:
				DialogueManager.show_dialogue_balloon(NINA_DIA_7_2, "start")


func _on_area_entered(area: Area2D) -> void:
	sprite_2d.visible = true
	player_close = true

func _on_area_exited(area: Area2D) -> void:
	sprite_2d.visible = false
	player_close = false

func _on_dialogue_started(dialogue):
	GameManager.is_dialogue_active = true

func _on_dialogue_ended(dialogue):
	await get_tree().create_timer(0.2).timeout
	GameManager.is_dialogue_active = false
